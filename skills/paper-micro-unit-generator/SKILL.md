...
- `scripts/generate_all_offline.py`
  ...
  - 做什么：读取 `paper_output/plan/model_recommendations.json`、清洗数据概况和占位符，生成较完整的通用论文微单元，写入 `paper_output/micro_units/*.txt` 和 `paper_output/generate_log.json`。
- `scripts/merge.py`
...
- 可选：
  ...
  - 模型推荐结果：`paper_output/plan/model_recommendations.json`，由 `modeling-paper-rubric-and-model-selector/scripts/recommend_models.py` 生成，用于指导基线、改进、指标与验证描述。
...
3. 运行 `python ~/.codex/skills/paper-micro-unit-generator/scripts/generate_all_offline.py` 生成 `paper_output/micro_units/*.txt`；
4. 运行 `python ~/.codex/skills/paper-micro-unit-generator/scripts/merge.py` 生成 `paper_output/final_paper.md`；
