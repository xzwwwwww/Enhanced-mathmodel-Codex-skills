...
## 审计维度（可逐层开关）
开始审计前，若任务是数学建模论文、建模路线或最终稿，优先读取 `references/qa_checklist.md`，并把其中“必过项”作为阻塞门禁。
...
**最终论文审计**：
```bash
python ~/.codex/skills/quality-assurance-auditor/scripts/audit_paper.py
```

**行为**：检查 `problem_files/`、`tasks.json`、微单元、`final_paper.md`、图表引用、占位符、重复段落、基线/验证描述、`final_paper.docx`，并生成 `paper_output/qa_report.md/json`。若有阻塞问题，脚本返回非零码。
